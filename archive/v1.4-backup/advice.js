...
function generateAdvice() {
  ...
  if(u.goal==='cut') {
    ...
    <p><strong>🎯 热量目标</strong>：建议每日摄入 <span style="color:var(--success)">${formatEnergy(Math.round(tdee*0.8))}</span>（TDEE ${formatEnergy(tdee)} 的80%）。</p>
    ...
    <p><strong>♀️ 月经安全线</strong>：瘦体重${lbm.toFixed(1)}kg × 30 = ${formatEnergy(safeCal)}。每日摄入不应低于此值。</p>
    ...
    <p>根据您的档案（${u.name}，${u.weight}kg，体脂${u.bodyfat}%），以下是针对性建议：</p>
    <p><strong>🎯 热量目标</strong>：${u.goal==='bulk'?'建议每日摄入 '+formatEnergy(Math.round(tdee*1.15)):'建议维持 '+formatEnergy(tdee)+' 左右'}。</p>
...
function copyToWorkBuddy() {
  ...
  const summary = dietFoods.map(d => {
    ...
    return f ? `${f.name} ${d.amount}g (${formatEnergy(Math.round(parseFloat(f.energyKCal||0)*d.amount/100))})` : '';
  }).join('\n');
